...
MODULE_PARM_DESC(BAUDRATE,
                 " Baudrate value [default = 4800] (min=1200, max=19200)");
...
{
    pGPIO_REGISTER->GPFSEL[gpio / 10] =
      (pGPIO_REGISTER->GPFSEL[gpio / 10] & ~(0b111 << ((gpio % 10) * 3)))
      | ((function << ((gpio % 10) * 3)) & (0b111 << ((gpio % 10) * 3)));
}
...
{
    if (gpio > 31)
        return (pGPIO_REGISTER->GPLEV[gpio / 32] & (0x00000001 << (gpio - 32)))
               >> (gpio - 32);
    else
        return (pGPIO_REGISTER->GPLEV[gpio / 32] & (0x00000001 << gpio))
               >> gpio;
}
...
static enum hrtimer_restart FunctionTimerTX(struct hrtimer *unused)
{
    ...
    if (strlen(TX_BUFFER) > 0)  // Data ready to send
    {
        ...
        {
            GPIOOutputValueSet(GPIO_TX,
                               ((TX_BUFFER[strlen(TX_BUFFER) - 1] & (1 << bit))
                                >> bit));
            bit++;
        } else if (bit == 8) {
...
//--------------------------------------------------------------------------------------
static ssize_t set_gpio_tx_callback(struct device *dev,
...
//--------------------------------------------------------------------------------------
static ssize_t set_gpio_rx_callback(struct device *dev,
                                    struct device_attribute *attr,
                                    const char *buf,
                                    size_t count)
{
...
//--------------------------------------------------------------------------------------
static ssize_t get_gpio_tx_callback(struct device *dev,
                                    struct device_attribute *attr,
                                    char *buf,
                                    size_t count)
{
...
//--------------------------------------------------------------------------------------
static ssize_t get_gpio_rx_callback(struct device *dev,
                                    struct device_attribute *attr,
                                    char *buf,
                                    size_t count)
{
...
//--------------------------------------------------------------------------------------
static ssize_t get_data_callback(struct device *dev,
                                 struct device_attribute *attr,
                                 char *buf,
                                 size_t count)
{
...
//--------------------------------------------------------------------------------------
static ssize_t set_loopback_callback(struct device *dev,
                                     struct device_attribute *attr,
                                     const char *buf,
                                     size_t count)
{
...
//--------------------------------------------------------------------------------------
static ssize_t get_loopback_callback(struct device *dev,
                                     struct device_attribute *attr,
                                     char *buf,
                                     size_t count)
{
...
//--------------------------------------------------------------------------------------
static ssize_t set_baudrate_callback(struct device *dev,
                                     struct device_attribute *attr,
                                     const char *buf,
                                     size_t count)
{
...
//--------------------------------------------------------------------------------------
static ssize_t get_baudrate_callback(struct device *dev,
                                     struct device_attribute *attr,
                                     char *buf,
                                     size_t count)
{
...
static DEVICE_ATTR(loopback,
                   0644,
                   get_loopback_callback,
                   set_loopback_callback);
static DEVICE_ATTR(baudrate,
                   0644,
                   get_baudrate_callback,
                   set_baudrate_callback);
